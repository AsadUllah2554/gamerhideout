  ...
  const acceptInvite = async (chatId) => {
    try {
      const response = await axios.patch(
        `${process.env.SERVER_URL}api/chat/invitation/accept`,
        {
          chatId: chatId,
          userId: user._id,
        }
  ...
  const inviteMember = async () => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:5000/api/chat/invite`,
        {
          chatId: chat._id,
    ...
    const fetchPendingInvites = async () => {
      try {
        const { data } = await axios.get(
          `http://localhost:5000/api/chat/invitations/${currentUser}`
        );
      } catch (error) {
        console.log(error);
      }
          ...
          <div className="pending-invites-container">
            {pendingInvites.map((invite) => (
              <div key={invite._id} className="pending-invite-item">
                ...
                <p>
                  {invite.semester} {invite.section}
                </p>
