...
const Chat = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`http://localhost:5000/api/chat/creategroup`, {
        creatorId: user._id,
        groupName: groupName,
        semester: semester,
        section: section,
  ...
  useEffect(() => {
    ...
    const fetchUserGroups = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/chat/groups/${user._id}`);
        console.log('User groups: ', response.data.userGroups);
        const userAcceptedGroups = response.data.userGroups.filter(group => (
          group.members.some(member => member.user._id === user._id && member.status === "accepted")
        ));
  ...
  useEffect(() => {
    const fetchPendingInvites = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/chat/invitations/${user._id}`);
    ...
    <div className="Chat">
      ...
      <div className="Left-side-chat">
        ...
        <div className="Chat-container">
          <h2>Chats</h2>
          <p onClick={handlePendingInvites}>Pending invitations {pendingInvites.length}</p>
  { user.cr && <p onClick={handleModalOpen}>Create new group</p>}
  <ReactModal isOpen={isModalOpen} onRequestClose={handleModalClose}  style={{
    content: {
